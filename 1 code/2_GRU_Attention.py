...
class myGRU(nn.Module):
    ...
    def forward(self, input):
        ...
        tanh = nn.Tanh()
        atten_weight = F.softmax(tanh(self.attn(r_out)), dim=1)
        w1 = atten_weight[:, 0, 0]
        w2 = atten_weight[:, 1, 0]
        w3 = atten_weight[:, 2, 0]
        w4 = atten_weight[:, 3, 0]
        ...
        out = self.out(atten_applied.squeeze(1))
        return out
...
if __name__ == '__main__':
    ...
    loss_csv.to_csv('loss_save_2_GRU_Attention.csv')

    # 保存完整模型
    torch.save(gru, '2_GRU_Attention.pkl')
    ...
    pred.to_csv('pred_value_2_GRU_Attention.csv')
