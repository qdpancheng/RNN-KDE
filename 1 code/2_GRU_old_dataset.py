...
if __name__ == '__main__':
    ...
    data = pd.read_excel(filePath + r'/data_20150401_20160531.xlsx')

    # 数据预处理--进行数据归一化 Standardize features by removing the mean and scaling to unit variance
    data_temp = np.zeros((120384, 5))           # 本次实验使用5个天气变量值，故data_temp的第二个维度设置为5
    data_temp = data.iloc[:, 3: -1].values      # 取出天气变量进行归一化处理
    ss = StandardScaler()
    ...
    # 实际预测时间段 5:30 -- 19:00
    # 366 + 30 + 31 - 9 = 418天(去掉了数据缺失的天)  solar power + 5个天气变量 = 6
    data_processed = np.zeros((418, 168, 6))
    for ii in range(418):
        temp = data.iloc[288 * ii + 60: 288 * ii + 228, 2: -1].values
        data_processed[ii, :, :] = temp

    print(data_processed.shape)
    train = data_processed[:357, :, :]
    val = data_processed[357: 387, :, :]
    test = data_processed[387: 418, :, :]
    ...
    train_processed = np.zeros((357 * 162, SEQ + 1, 6))       # 05:30 -- 19:00 共162个点(待预测点个数--一天产生的样本)
    for ii in range(357):
        for jj in range(162):
            train_processed[ii * 162 + jj, :, :] = train[ii, jj + 2: jj + 3 + SEQ, :]    # 从05:00开始,取过去4个值进行预测,预测从05：30开始，故jj + 2

    val_processed = np.zeros((30 * 162, SEQ + 1, 6))
    for ii in range(30):
        for jj in range(162):
            val_processed[ii * 162 + jj, :, :] = val[ii, jj + 2: jj + 3 + SEQ, :]

    test_processed = np.zeros((31 * 162, SEQ + 1, 6))
    for ii in range(31):
        for jj in range(162):
    ...
    HIDDEN_SIZE = [50 + 10 * ii for ii in range(1)]
    NUM_LAYERS = [2 + ii for ii in range(1)]
    ...
    for qqq, Hidden_Size in enumerate(HIDDEN_SIZE):
        for www, Num_layer in enumerate(NUM_LAYERS):
            ...
            if (TORCH_GPU == 0):
                ...
                gru = myGRU(Hidden_Size=Hidden_Size, Num_Layers=Num_layer).cuda(GPU_NUM)
            print(gru)
            # 定义优化器
    ...
    # ==================== 以下为使用超参数确定模型并进行训练过程 ========================================
    if (TORCH_GPU == 0):
        gru = myGRU(Hidden_Size=60, Num_Layers=3)
    else:
        gru = myGRU(Hidden_Size=60, Num_Layers=3).cuda(GPU_NUM)
    print(gru)
    ...
    loss_csv.to_csv('loss_save_2_GRU_weather_solar_record_1.csv')

    # 保存完整模型
    torch.save(gru, '2_GRU.pkl')
    ...
    pred.to_csv('pred_value_2_GRU_weather_solar_record_1.csv')

    print(pred_test.shape)
    pred = np.reshape(pred_test, (-1))
    Y_test = np.reshape(Y_test, (-1))
    error_RMSE = np.sqrt(np.mean(np.square(Y_test - pred)))
    print(error_RMSE)
    # plt.plot(pred, 'r')
    # plt.plot(Y_test)
